...
})({"ha4vU":[function(require,module,exports) {
"use strict";
var HMR_HOST = null;
var HMR_PORT = null;
...
let runningTotal = new _decimalJs.Decimal(0);
let currentOperand = new _decimalJs.Decimal(0);
let lastAction = _button.Button.EQUALS;
let outputValue = "0";
...
function writeOutput() {
    if (outputValue.length > 13) {
        if (_button.Button.isInput(lastAction)) outputValue = outputValue.substr(0, 13);
        else {
            clear();
            outputValue = "ERROR";
        }
...
function handleNumber(inputValue) {
    if (_button.Button.isInput(lastAction)) {
        if (outputValue === "0") outputValue = inputValue;
        else outputValue += inputValue;
    } else if (_button.Button.isOperator(lastAction)) outputValue = inputValue;
    else {
        clear();
...
function handleOperator(inputValue) {
    if (_button.Button.isInput(lastAction)) {
        ...
        currentOperand = new _decimalJs.Decimal(outputValue);
        execute();
    }
...
function clear() {
    ...
    runningTotal = new _decimalJs.Decimal(0);
    currentOperand = new _decimalJs.Decimal(0);
    outputValue = "0";
}
function backspace() {
    if (lastAction == _button.Button.EQUALS) clear();
    else if (!_button.Button.isInput(lastAction) || outputValue.length <= 1) outputValue = "0";
    else outputValue = outputValue.slice(0, -1);
}
function equals() {
    if (_button.Button.isOperator(lastAction)) return;
    if (_button.Button.isInput(lastAction)) currentOperand = new _decimalJs.Decimal(outputValue);
    execute();
}
function execute() {
    switch(selectedOperator){
        case _button.Button.DIVIDE:
            runningTotal = runningTotal.dividedBy(currentOperand);
            break;
        case _button.Button.MULTIPLY:
            runningTotal = runningTotal.times(currentOperand);
            break;
        case _button.Button.SUBTRACT:
            runningTotal = runningTotal.minus(currentOperand);
            break;
        case _button.Button.ADD:
...
function init() {
    document.querySelector(".calc").addEventListener("click", function(event) {
        if (event.target.tagName === "BUTTON") buttonClicked(event.target.value);
    });
}
init();
function buttonClicked(buttonValue) {
    ...
    writeOutput();
    console.log(`State after "${buttonValue}": operator: ${selectedOperator}, total: ${runningTotal.toString()}, operand: ${currentOperand.toString()}, last action: ${lastAction}`);
}
function inputHandler(buttonValue) {
    if (_button.Button.isNumber(buttonValue)) handleNumber(buttonValue);
    else if (_button.Button.isOperator(buttonValue)) handleOperator(buttonValue);
    else if (buttonValue == _button.Button.CLEAR) clear();
    else if (buttonValue == _button.Button.BACKSPACE) backspace();
    else if (buttonValue == _button.Button.EQUALS) equals();
    else if (buttonValue == _button.Button.DECIMAL) outputValue += ".";
    else {
...
},{"./button":"4Ifff","decimal.js":"gUkOJ"}],"4Ifff":[function(require,module,exports) {
...
const Button = Object.freeze({
    ...
    isInput (value) {
        return [
            this.BACKSPACE,
            this.DECIMAL
        ].includes(value) || this.isNumber(value);
    }
});
...
},{}],"gUkOJ":[function(require,module,exports) {
...
},{}]},["ha4vU","9WiAu"], "9WiAu", "parcelRequireff1f")

//# sourceMappingURL=index.6ded4761.js.map
